...
public class HotelImageControllerTests {
	...
	public void testGetAllImages () throws Exception {
		// Mock data
		HotelImageDTO image1 = new HotelImageDTO(1L, "image1.jpg", "image/jpeg", null);
		HotelImageDTO image2 = new HotelImageDTO(2L, "image2.jpg", "image/png", null);
		...
		// Perform GET request
		mockMvc.perform(get("/api/hotel-images")).andExpect(status().isOk()).andExpect(
				content().contentType(MediaType.APPLICATION_JSON)).andExpect(jsonPath("$[0].id").value(1)).andExpect(
				jsonPath("$[0].imageName").value("image1.jpg")).andExpect(
				jsonPath("$[0].imageType").value("image/jpeg")).andExpect(
				jsonPath("$[0].imageUrl").value("/api/hotel-images/1")).andExpect(
				jsonPath("$[1].id").value(2)).andExpect(jsonPath("$[1].imageName").value("image2.jpg")).andExpect(
				jsonPath("$[1].imageType").value("image/png")).andExpect(
				jsonPath("$[1].imageUrl").value("/api/hotel-images/2"));
	}
	...
	public void testGetImage () throws Exception {
		...
		// Perform GET request
		mockMvc.perform(get("/api/hotel-images/{imageId}", imageId)).andExpect(status().isOk()).andExpect(
				content().contentType(MediaType.parseMediaType(image.getImageType()))).andExpect(
				header().string(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"image1.jpg\"")).andExpect(
				content().bytes(image.getImageBase64()));
	}
