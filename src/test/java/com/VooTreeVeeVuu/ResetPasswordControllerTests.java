...
public class ResetPasswordControllerTests {
    ...
    public void testForgotPassword_Success() throws Exception {
        String email = "nguyenttpc04898@fpt.edu.vn";
    ...
    @Test
    public void testForgotPassword_BadRequest() throws Exception {
        String email = "nguyenttpc04898@fpt.edu.vn";
        String errorMessage = "Email not found";

        // Mock service to throw exception (bad request case)
        Mockito.doThrow(new IllegalArgumentException(errorMessage)).when(accountService).generateAndSendOTP(email);

        // Send request to POST /forgot and expect bad request response
        mockMvc.perform(MockMvcRequestBuilders.post(END_POINT_PATH+"/forgot")
                        .param("email", email)
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    public void testResetPassword_BadRequest() throws Exception {
        ...
        // Mock service to throw exception (bad request case)
        Mockito.doThrow(new IllegalArgumentException(errorMessage)).when(accountService).resetPassword(email, otp, newPassword);

        // Send request to POST /reset and expect bad request response
        mockMvc.perform(MockMvcRequestBuilders.post(END_POINT_PATH+"/reset")
                        .param("email", email)
                        .param("otp", otp)
                        .param("newPassword", newPassword)
